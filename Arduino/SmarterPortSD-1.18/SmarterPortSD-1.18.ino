...
// 1.16 - Load/Save Boot Partition Info from SD CARD instead of EEPROM. (NORTHERNDIY)
// 1.17 - SRAM Optimizations (Datatype selection), Serial Debug now optional.  (NORTHERNDIY)
// 1.18 - NUMPARTITIONS is now up to 5, but loads # of disks presented to IIGS from config file (1-5).  (NORTHERNDIY)

//NOTES:
//    -Can not boot from SMARTPORTSD if attached in chain after another disk drive.
//    -The interface is pretty slow, but with the optimizations made in 1.17 and 1.18, the drive is quicker than it was in 1.15, but still slow!
//    -Limited to 32MB partitions, in theory this could be expanded if we allow HFS partitions, but to conserve memory this is limited to 32mb!
//    -If no Smartport.cfg file is on the sd card, a default one is created and written to it.
//
...
//
//
// (Serial Debug support now made optional through ifdef below - Saves 10 bytes ram, and 3KB Flash)
//
...
//Can Disable debug and serial output/library inclusions.
#define DEBUGPRNT 0
#if DEBUGPRNT ==1
#define debug(x) Serial.print(x)
#define debugD(x,y) Serial.print(x,y)
#define debugln(x) Serial.println(x)
#define debugBegin(x) Serial.begin(x)
#define debug_sprintf(x,y,z) sprintf(x,y,z)
#else
#define debug(x)
#define debugD(x,y)
#define debugln(x)
#define debugBegin(x)
#define debug_sprintf(x,y,z)
#endif
...
//This just allocates memory in advance so we can avoid malloc ilk.
//Actul number of partitions presented to IIGS is loaded from config file.
#define MAX_PARTITIONS  4          // Maximum Number of 32MB Prodos partions supported is 5 (limited by SRAM available)
...
uint8_t NUM_PARTITIONS = MAX_PARTITIONS;  //We actually restrict this from the config file later, but this ensures
//The compiler allocates memory appropriately without malloc
...
struct device{
  ...
  //Unused items
  //uint16_t header_offset:10;      //Some image files have headers, skip this many bytes to avoid them
  //bool online;                    //Whether this image is currently available
                                    //No longer used, user devices[...].sdf.isOpen() instead
  //bool writeable;                 //Unused
};

uint8_t const partition_led_pins[4] = {14, 15, 16, 8};
...
void setup() {
  ...
  //bootPartition = eeprom_read_byte(0);
  //if (bootPartition == 0xFF) bootPartition = 0;

  //Grab boot partition from configuration file
  if(sdcard.exists("SmartPort.cfg")){
    File config_file = sdcard.open("SmartPort.cfg", O_READ);
    bootPartition = config_file.read() - '0'; //Disgusting hack, but no memory overhead!
    NUM_PARTITIONS = config_file.read() - '0';
    debug(F("\r\nConfigRead (RAW): boot: "));
    debugD(bootPartition,DEC);
    debug(F("\r\nConfigRead (RAW): MAX_PARTITIONS: "));
    debugD(NUM_PARTITIONS,DEC);
    config_file.close();  
  }else{//Config File doesn't exist so create a default one
    File config_file = sdcard.open("SmartPort.cfg", O_CREAT | O_WRITE);
    config_file.write("0");
    config_file.write("4");
    config_file.close();
    bootPartition = 0;
  ...
  bootPartition = (bootPartition % 4);
  NUM_PARTITIONS = (NUM_PARTITIONS % (MAX_PARTITIONS + 1));
  debug(F("\r\nBoot partition: "));
  debugD(bootPartition, DEC);
  digitalWrite(partition_led_pins[bootPartition], HIGH);
  ...
  while (1) {
    ...
    switch (phases) {
      ...
      case 0x05:
        ...
        //debug(number_partitions_initialised);
      ...
      case 0x0e:
      case 0x0f:
        //debug(("E ")); //this is timing sensitive, so can't print to much here as it takes to long
        noInterrupts();
        ...
        if (packet_buffer[14] != 0x85)  // if its an init pkt, then assume its for us and continue on
        {
          ...
          {
            if ( devices[(partition + bootPartition) % NUM_PARTITIONS].device_id != packet_buffer[6])  //destination id
              noid++;
          }
        ...
        //debugln(packet_buffer[8], HEX);
        //debugln(packet_buffer[14], HEX);
        /*
        ...
        switch (packet_buffer[14]) {
          ...
          case 0x80:  //is a status cmd
            ...
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + bootPartition) % NUM_PARTITIONS].device_id == source
                && devices[(partition + bootPartition) % NUM_PARTITIONS].sdf.isOpen() ) {  //yes it is, and it's online, then reply
                ...
                if (status_code == 0x03) { // if statcode=3, then status with device info block
                  debug(F("\r\n******** Sending DIB! ********"));
                  encode_status_dib_reply_packet(devices[(partition + bootPartition) % NUM_PARTITIONS]);
                  //print_packet ((unsigned char*) packet_buffer,packet_length());
                  delay(50);
                  ...
                  debug(F("\r\nSource: "));
                  debug(source,HEX);
                  debug(F(" Partition ID: "));
                  debug(devices[(partition + bootPartition) % NUM_PARTITIONS].device_id, HEX);
                  debug(F(" Status code: "));
                  debug(status_code, HEX);
                  */
                  encode_status_reply_packet(devices[(partition + bootPartition) % NUM_PARTITIONS]);        
                }
                noInterrupts();
                DDRD = 0x40; //set rd as output
                status = SendPacket( (unsigned char*) packet_buffer);
          ...
          case 0xC0:  //Extended status cmd
            ...
            //debugln(source, HEX);
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + bootPartition) % NUM_PARTITIONS].device_id == source) {  //yes it is, then reply
                //Added (unsigned short) cast to ensure calculated block is not underflowing.
                status_code = (packet_buffer[21] & 0x7f);
                 debug(F("\r\nExtended Status CMD:"));
                 debugD(status_code, HEX);
                 print_packet ((unsigned char*) packet_buffer,packet_length());
                 if (status_code == 0x03) { // if statcode=3, then status with device info block
                  debugln(F("Extended status DIB!"));
                } else {  // else just return device status
                  ...
                  //debug(status_code, HEX);
                  //delay(50);
                  encode_extended_status_reply_packet(devices[(partition + bootPartition) % NUM_PARTITIONS]);        
                }
                noInterrupts();
                DDRD = 0x40; //set rd as output
                status = SendPacket( (unsigned char*) packet_buffer);
          ...
          case 0xC1:  //extended readblock cmd
            ...
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + bootPartition) % NUM_PARTITIONS].device_id == source) {  //yes it is, then do the read
                // block num 1st byte
                //Added (unsigned short) cast to ensure calculated block is not underflowing.
                ...
                // block_num = block_num + (((partition + bootPartition) % 4) * 65536);
                ...
                if (!devices[(partition + bootPartition) % NUM_PARTITIONS].sdf.seekSet(block_num*512)){
                  debug(F("\r\nRead err!"));
                }
                
                sdstato = devices[(partition + bootPartition) % NUM_PARTITIONS].sdf.read((unsigned char*) packet_buffer, 512);    //Reading block from SD Card
                if (!sdstato) {
                  debug(F("\r\nRead err!"));
                }
            ...
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + bootPartition) % NUM_PARTITIONS].device_id == source) {  //yes it is, then do the read
                ...
                if (!devices[(partition + bootPartition) % NUM_PARTITIONS].sdf.seekSet(block_num*512)){
                  debug(F("\r\nRead seek err!"));
                  debug(F("\r\nPartition #"));
                  debug((partition + bootPartition) % NUM_PARTITIONS);
                  debug(F(" block #"));
                  debug(block_num);
                  if(devices[(partition + bootPartition) % NUM_PARTITIONS].sdf.isOpen()){
          ...
          case 0x82:  //is a writeblock cmd
            source = packet_buffer[6];
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + bootPartition) % NUM_PARTITIONS].device_id == source) {  //yes it is, then do the write
                // block num 1st byte
                //Added (unsigned short) cast to ensure calculated block is not underflowing.
                ...
                if (status == 0) { //ok
                  ...
                  sdstato = devices[(partition + bootPartition) % NUM_PARTITIONS].sdf.write((unsigned char*) packet_buffer, 512);   //Write block to SD Card
                  if (!sdstato) {
                    debug(F("\r\nWrite err!"));
                    //debug(F(" Block n.:"));
                    //debug(block_num);
          ...
          case 0x83:  //is a format cmd
            source = packet_buffer[6];
            for (partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
              if (devices[(partition + bootPartition) % NUM_PARTITIONS].device_id == source) {  //yes it is, then reply to the format cmd
          ...
          case 0x85:  //is an init cmd
            ...
            if (number_partitions_initialised < NUM_PARTITIONS) { //are all init'd yet
              devices[(number_partitions_initialised - 1 + bootPartition) % NUM_PARTITIONS].device_id = source; //remember source id for partition
            ...
            if (number_partitions_initialised - 1 == NUM_PARTITIONS) {
              for (partition = 0; partition < NUM_PARTITIONS; partition++) {
                debug(F("\r\nDrive: "));
                debugD(devices[(partition + bootPartition) % NUM_PARTITIONS].device_id, HEX);
              }
            }
            break;          
...
int verify_cmdpkt_checksum(void)
{
  ...
  //  debug(F("Pkt Chksum Byte:\r\n"));
  //  debug(pkt_checksum,DEC);
  //  debug(F("Calc Chksum Byte:\r\n"));
  //  debug(calc_checksum,DEC);
...
void print_packet (unsigned char* data, int bytes)
{
  ...
  for (count = 0; count < bytes; count = count + 16) {
    debug_sprintf(tbs, ("%04X: "), count);
    debug(tbs);
    for (row = 0; row < 16; row++) {
      if (count + row >= bytes)
        debug(F("   "));
      else {
        debugD(data[count + row], HEX);
        debug(" ");
      }
    }
    debug(F("-"));
...
int rotate_boot (void)
{
  ...
  for(i = 0; i < NUM_PARTITIONS; i++){
    ...
    if(devices[bootPartition].sdf.isOpen()){
  ...
  //eeprom_write_byte(0, bootPartition);
  File config_file = sdcard.open("SmartPort.cfg",O_WRITE);
  char config_partition = '0' + bootPartition;
  config_file.write(config_partition);
  config_file.close();
  ...
  //Close all the image files (and flush all their data to the card)
  for (i=0; i<NUM_PARTITIONS; i++){
    devices[i].sdf.close();
  }
  
 while (1){
   for (i=0;i<(bootPartition+1);i++) {
     digitalWrite(statusledPin,HIGH);
     digitalWrite(partition_led_pins[bootPartition], HIGH);
     delay(200);   
     digitalWrite(statusledPin,LOW);
     digitalWrite(partition_led_pins[bootPartition], LOW);
     delay(100);   
   }
   delay(600);
 }
...
bool open_image( device &d, String filename ){
  ...
  if(!d.sdf.isOpen()||!d.sdf.isFile()){
    ...
    debug(F("file before checking for valid image type!"));
    return false;
  }
...
bool is_ours(unsigned char source){
  for (unsigned char partition = 0; partition < NUM_PARTITIONS; partition++) { //Check if its one of ours
    if (devices[(partition + bootPartition) % NUM_PARTITIONS].device_id == source) {  //yes it is
      return true;
    }
  }
  return false;
...
void criticalError(){
      while(1){
      //Display.displayNum(0,5);
      //Display.displayNum(1,0x0d);
      //Display.displayNum(2,0x0e);
      digitalWrite(partition_led_pins[0],HIGH);
      digitalWrite(partition_led_pins[1],HIGH);
      digitalWrite(partition_led_pins[2],HIGH);
      digitalWrite(partition_led_pins[3],HIGH);
      digitalWrite(statusledPin, HIGH);
      delay(250);
      digitalWrite(partition_led_pins[0],LOW);
      digitalWrite(partition_led_pins[1],LOW);
      digitalWrite(partition_led_pins[2],LOW);
      digitalWrite(partition_led_pins[3],LOW);
      digitalWrite(statusledPin, LOW);
